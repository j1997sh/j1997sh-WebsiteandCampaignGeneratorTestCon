(async function(){
'use strict';
const sb=window.cpSupabase,grid=document.getElementById('domainGrid'),msg=document.getElementById('domainMessage'),dialog=document.getElementById('domainDialog');
...
function targetOptions(){const type=domainTargetType.value,rows=type==='website'?websites:campaigns;domainTargetId.innerHTML=rows.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}
function openDialog(){dialog.hidden=false;targetOptions();domainHostname.focus()}
function closeDialog(){dialog.hidden=true;domainForm.reset();targetOptions()}
addDomainButton.onclick=openDialog;domainDialogClose.onclick=closeDialog;domainCancel.onclick=closeDialog;domainTargetType.onchange=targetOptions;
domainForm.onsubmit=async e=>{e.preventDefault();const host=domainHostname.value.trim().toLowerCase().replace(/^https?:\/\//,'').replace(/\/.*$/,'');if(!host)return;const type=domainTargetType.value,id=domainTargetId.value;const payload={account_id:account.id,hostname:host,target_type:type,website_id:type==='website'?id:null,campaign_id:type==='campaign'?id:null,status:'pending',dns_instructions:{cname:{name:'www',value:'sites.campaignplatform.example'},a:{name:'@',value:'203.0.113.10'}}};const r=await sb.from('domains').insert(payload).select().single();if(r.error){show(r.error.message,true);return}domains.unshift(r.data);closeDialog();render()};
...
domainAccountName.textContent=account.name||'Signed in';domainAccountInitials.textContent=(account.name||'CP').split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase();
domainLogout.onclick=async e=>{e.preventDefault();await sb.auth.signOut();location.href='login.html'};render()
})();