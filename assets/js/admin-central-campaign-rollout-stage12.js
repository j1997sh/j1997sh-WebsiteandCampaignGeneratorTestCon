...
async function init(){
 ...
 let data=null,supporters=[],performance=[],facebookCreatives=[];
 ...
 async function load(){const [r,sr,pr,ar,cr]=await Promise.all([sb.rpc('org_admin_central_campaign_rollout',{p_rollout:id}),sb.rpc('org_admin_central_campaign_supporters',{p_rollout:id}),sb.rpc('org_admin_central_campaign_rollout_performance',{p_rollout:id}),sb.rpc('org_admin_facebook_audiences',{p_org:orgId}),sb.rpc('org_admin_facebook_creatives',{p_rollout:id})]);if(r.error||sr.error||pr.error||ar.error||cr.error)return msg((r.error||sr.error||pr.error||ar.error||cr.error).message,true);data=r.data;supporters=sr.data||[];performance=pr.data||[];facebookCreatives=cr.data||[];facebookAudienceRefs=new Map();applyStoredFacebookAudiences(ar.data||[]);render();}
 ...
 const creativeByArea=()=>new Map((facebookCreatives||[]).map(x=>[areaKey(x.area),x]));
 function facebookRows(settings){return (data?.sites||[]).map(site=>{const audienceFile=applyPattern(settings.audiencePattern,site),audienceRef=facebookAudienceRefs.get(audienceFile.toLowerCase())||facebookAudienceRefs.get(areaKey(site.area))||'';const link=site.domain?(/^https?:\/\//i.test(site.domain)?site.domain:`https://${site.domain}`):`${settings.baseUrl}/${site.slug}`;const tags=`utm_source=facebook&utm_medium=paid_social&utm_campaign=${encodeURIComponent(settings.campaign)}&utm_content=${encodeURIComponent(site.slug)}`;const row={};META_ADS_HEADERS.forEach(h=>row[h]='');Object.assign(row,{'Campaign Name':settings.campaign,'Campaign Status':'PAUSED','Special Ad Categories':settings.special,'Special Ad Category Country':settings.special==='issues_elections_politics'?settings.country:'','Campaign Objective':settings.objective,'Buying Type':'AUCTION','Campaign Daily Budget':settings.budgetLevel==='campaign'?settings.budget:'','Campaign Bid Strategy':'Highest volume or value','Campaign Start Time':fbDate(settings.start),'Campaign Stop Time':fbDate(settings.end),'Ad Set Run Status':'PAUSED','Ad Set Name':metaName(settings.label,site.area),'Ad Set Time Start':fbDate(settings.start),'Ad Set Time Stop':fbDate(settings.end),'Ad Set Daily Budget':settings.budgetLevel==='adset'?settings.budget:'','Link Object ID':settings.pageId,'Link':link,'Countries':settings.country,'Custom Audiences':audienceRef,'Optimization Goal':'LANDING_PAGE_VIEWS','Billing Event':'IMPRESSIONS','Ad Set Bid Strategy':settings.budgetLevel==='adset'?'Highest volume or value':'','Ad Status':'PAUSED','Ad Name':metaName(settings.label,site.area,'01'),'Title':applyPattern(settings.headline,site).replaceAll('{{area}}',site.area),'Body':applyPattern(settings.body,site).replaceAll('{{area}}',site.area),'Creative Type':'Page post ad','URL Tags':tags,'Image File Name':(creativeByArea().get(areaKey(site.area))?.filename||applyPattern(settings.imagePattern,site)),'Call to Action':settings.cta});const creative=creativeByArea().get(areaKey(site.area))||null;return {site,audienceFile,audienceRef,creative,row};})}
 function renderFacebookPreflight(){const s=facebookSettings(),rows=facebookRows(s),missingAudience=rows.filter(x=>!x.audienceRef).length,missingCreative=rows.filter(x=>!x.creative).length,missingDomain=rows.filter(x=>!x.row.Link).length,total=rows.length,totalBudget=Number(s.budget||0)*(s.budgetLevel==='adset'?total:1);facebookAdsPreflight.innerHTML=`<div class="facebook-preflight-kpis"><div><span>Ads</span><strong>${total}</strong></div><div><span>Audiences matched</span><strong>${total-missingAudience}/${total}</strong></div><div><span>Creatives matched</span><strong>${total-missingCreative}/${total}</strong></div><div><span>Daily budget</span><strong>£${totalBudget.toLocaleString(undefined,{maximumFractionDigits:2})}</strong></div></div>${(missingAudience||missingCreative)?`<div class="state-banner error">${missingAudience?`${missingAudience} audience${missingAudience===1?' is':'s are'} missing. `:''}${missingCreative?`${missingCreative} creative${missingCreative===1?' is':'s are'} missing.`:''}</div>`:'<div class="state-banner success">All audiences and creatives are matched and ready for Meta bulk upload.</div>'}<div class="facebook-preflight-table"><table><thead><tr><th>Area</th><th>Ad set</th><th>Landing page</th><th>Audience</th><th>Image</th></tr></thead><tbody>${rows.slice(0,8).map(x=>`<tr><td>${esc(x.site.area)}</td><td>${esc(x.row['Ad Set Name'])}</td><td><code>${esc(x.row.Link)}</code></td><td class="${x.audienceRef?'':'facebook-missing'}">${esc(x.audienceRef||x.audienceFile+' — missing')}</td><td class="${x.creative?'':'facebook-missing'}"><code>${esc(x.row['Image File Name'])}</code>${x.creative?'':' — missing'}</td></tr>`).join('')}</tbody></table>${rows.length>8?`<small>Showing 8 of ${rows.length} ads.</small>`:''}</div>`;}
 ...
 manageFacebookCreatives.href=`admin-facebook-creatives.html?rollout=${encodeURIComponent(id)}`;
 ...
 facebookAdsForm.onsubmit=async e=>{
   e.preventDefault();const settings=facebookSettings();
   if(!settings.pageId)return msg('Facebook Page ID is required for Page post ads.',true);
   if(!settings.baseUrl||settings.baseUrl==='https:/')return msg('Add the landing page base URL before exporting.',true);
   const rows=facebookRows(settings),missing=rows.filter(x=>!x.audienceRef||!x.creative);
   const csv=[META_ADS_HEADERS.map(csvCell).join(','),...rows.map(x=>META_ADS_HEADERS.map(h=>csvCell(x.row[h])).join(','))].join('\r\n');
   const manifest=(headers,vals)=>[headers.map(csvCell).join(','),...vals.map(r=>headers.map(h=>csvCell(r[h])).join(','))].join('\r\n');
   const audienceManifest=manifest(['area','audience_file','audience_ref'],rows.map(x=>({area:x.site.area,audience_file:x.audienceFile,audience_ref:x.audienceRef})));
   const creativeManifest=manifest(['area','filename','storage_path','matched'],rows.map(x=>({area:x.site.area,filename:x.row['Image File Name'],storage_path:x.creative?.storage_path||'',matched:x.creative?'yes':'no'})));
   const landingManifest=manifest(['area','slug','landing_url','ad_set_name'],rows.map(x=>({area:x.site.area,slug:x.site.slug,landing_url:x.row.Link,ad_set_name:x.row['Ad Set Name']})));
   const missingReport=manifest(['area','missing_audience','missing_creative'],missing.map(x=>({area:x.site.area,missing_audience:x.audienceRef?'':'yes',missing_creative:x.creative?'':'yes'})));
   if(!window.JSZip)return msg('ZIP packaging library did not load. Refresh and try again.',true);
   const zip=new JSZip();zip.file('meta-bulk-upload.csv',csv);zip.file('audience-manifest.csv',audienceManifest);zip.file('creative-manifest.csv',creativeManifest);zip.file('landing-page-manifest.csv',landingManifest);zip.file('missing-items.csv',missingReport);
   const creativeFolder=zip.folder('creatives');
   for(const x of rows.filter(x=>x.creative)){
     const d=await sb.storage.from('campaign-assets').download(x.creative.storage_path);
     if(d.error){console.warn('Creative download failed',x.creative.storage_path,d.error);continue}
     creativeFolder.file(x.creative.filename,d.data);
   }
   const blob=await zip.generateAsync({type:'blob'});
   const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${(settings.campaign||'facebook-campaign').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-meta-upload-package.zip`;a.click();URL.revokeObjectURL(a.href);
   msg(`${rows.length} Facebook ads packaged with audiences, creatives and landing-page manifests.${missing.length?` ${missing.length} row${missing.length===1?' needs':'s need'} attention.`:''}`,!!missing.length);
   facebookAdsDialog.close();
 };
